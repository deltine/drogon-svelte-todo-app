...
function build_drogon() {
    ...
    if [ $1 -eq 1 ]; then
        cmake .. -DBUILD_TESTING=YES $cmake_gen -DCMAKE_CXX_FLAGS="-std=c++20"
    elif [ $1 -eq 2 ]; then
        cmake .. -DBUILD_TESTING=YES -DBUILD_DROGON_SHARED=ON -DCMAKE_CXX_VISIBILITY_PRESET=hidden -DCMAKE_VISIBILITY_INLINES_HIDDEN=1 $cmake_gen -DCMAKE_CXX_FLAGS="-std=c++20"
    else
        cmake .. -DCMAKE_BUILD_TYPE=release $cmake_gen -DCMAKE_CXX_FLAGS="-std=c++20"
    fi
